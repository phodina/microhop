...
fn main() -> Result<(), Error> {
    ...
    let final_root = if let Some(overlay_cfg) = cfg.get_overlayfs() {
        ...
        let overlay_dev_path = resolve_device_path(&overlay_cfg.device, &blkid)
            .ok_or_else(|| {
                Error::new(std::io::ErrorKind::NotFound,
                    format!("Could not resolve overlayfs device: {}", overlay_cfg.device))
            })?;
